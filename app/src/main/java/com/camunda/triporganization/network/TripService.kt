...
interface TripService {
    ...
    suspend fun userLogIn(
        @Body user: UserLogInResponse,
    ): Response<Boolean>

    @POST("/trip/create")
    suspend fun createTripInstance(
        @Query("coordinatorId") userId: String,
    ): Response<Long>
    ...
    suspend fun saveTripCreationData(
        @Path("processInstanceKey") processKey: Long,
        @Body tripRequest: Trip,
    )

    @GET("/user/{userId}/tasks/all")
    suspend fun getActiveTripsForUser(
        @Path("userId") userId: String,
    ): Map<String, List<BasicTaskItem>>

    @GET("/offer/trip/{processKey}")
    suspend fun getOffersForTrip(
        @Path("processKey") processKey: Long,
    ): PartnerOfferResponse

    @POST("/offer/trip/{processKey}/accept")
    suspend fun acceptOffersForTrip(
        @Path("processKey") processKey: Long,
        @Query("transportPartnerIds") transportPartnerId: List<Long>,
        @Query("accommodationPartnerIds") accommodationPartnerId: List<Long>,
    )

    @POST("/offer/trip/{processKey}/rejectAll")
    suspend fun rejectOffersForTrip(
        @Path("processKey") processKey: Long,
    )

    @GET("/user/availableGuides")
    suspend fun getAvailableGuides(
        @Query("processKey") processKey: Long,
    ): List<String>

    @POST("/trip/{processKey}/assignTourGuide")
    suspend fun assignTourGuide(
        @Path("processKey") processKey: Long,
        @Query("tourGuide") tourGuide: String,
    )

    @GET("/trip/{processKey}/tripItinerary")
    suspend fun getTripItinerary(
        @Path("processKey") processKey: Long,
    ): Response<TripItinerary>

    @POST("/trip/{processKey}/reviewTripItinerary")
    suspend fun reviewTripItinerary(
        @Path("processKey") processKey: Long,
    ...
    suspend fun fillTripItinerary(
        @Path("processInstanceKey") processKey: Long,
        @Body tripItinerary: List<CitiesData>,
    )
